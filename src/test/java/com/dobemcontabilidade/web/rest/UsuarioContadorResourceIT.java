...
class UsuarioContadorResourceIT {
    ...
    void createUsuarioContador() throws Exception {
        ...
        var returnedUsuarioContador = om.readValue(
            restUsuarioContadorMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(usuarioContador)))
    ...
    @Test
    @Transactional
    void getAllUsuarioContadorsByAtivoIsEqualToSomething() throws Exception {
        // Initialize the database
        insertedUsuarioContador = usuarioContadorRepository.saveAndFlush(usuarioContador);

        // Get all the usuarioContadorList where ativo equals to
        defaultUsuarioContadorFiltering("ativo.equals=" + DEFAULT_ATIVO, "ativo.equals=" + UPDATED_ATIVO);
    }

    @Test
    @Transactional
    void getAllUsuarioContadorsByAtivoIsInShouldWork() throws Exception {
        // Initialize the database
        insertedUsuarioContador = usuarioContadorRepository.saveAndFlush(usuarioContador);

        // Get all the usuarioContadorList where ativo in
        defaultUsuarioContadorFiltering("ativo.in=" + DEFAULT_ATIVO + "," + UPDATED_ATIVO, "ativo.in=" + UPDATED_ATIVO);
    }

    @Test
    @Transactional
    void getAllUsuarioContadorsByAtivoIsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedUsuarioContador = usuarioContadorRepository.saveAndFlush(usuarioContador);

        // Get all the usuarioContadorList where ativo is not null
        defaultUsuarioContadorFiltering("ativo.specified=true", "ativo.specified=false");
    }
    ...
    void putExistingUsuarioContador() throws Exception {
        ...
        restUsuarioContadorMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedUsuarioContador.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedUsuarioContador))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingUsuarioContador() throws Exception {
        ...
        restUsuarioContadorMockMvc
            .perform(
                put(ENTITY_API_URL_ID, usuarioContador.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(usuarioContador))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamUsuarioContador() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUsuarioContadorMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(usuarioContador)))
    ...
    void patchNonExistingUsuarioContador() throws Exception {
        ...
        restUsuarioContadorMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, usuarioContador.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(usuarioContador))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamUsuarioContador() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUsuarioContadorMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(usuarioContador)))
            .andExpect(status().isMethodNotAllowed());
