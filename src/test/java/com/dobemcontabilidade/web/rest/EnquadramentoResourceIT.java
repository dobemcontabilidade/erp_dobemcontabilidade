...
class EnquadramentoResourceIT {
    ...
    void createEnquadramento() throws Exception {
        ...
        EnquadramentoDTO enquadramentoDTO = enquadramentoMapper.toDto(enquadramento);
        var returnedEnquadramentoDTO = om.readValue(
            restEnquadramentoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(enquadramentoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EnquadramentoDTO.class
        );

        // Validate the Enquadramento in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEnquadramento = enquadramentoMapper.toEntity(returnedEnquadramentoDTO);
    ...
    void putExistingEnquadramento() throws Exception {
        ...
        updatedEnquadramento
            ...
            .limiteFinal(UPDATED_LIMITE_FINAL)
            .descricao(UPDATED_DESCRICAO);
        EnquadramentoDTO enquadramentoDTO = enquadramentoMapper.toDto(updatedEnquadramento);

        restEnquadramentoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, enquadramentoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(enquadramentoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEnquadramento() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEnquadramentoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(enquadramentoDTO)))
    ...
    void partialUpdateEnquadramentoWithPatch() throws Exception {
        ...
        partialUpdatedEnquadramento.nome(UPDATED_NOME).sigla(UPDATED_SIGLA).descricao(UPDATED_DESCRICAO);
    ...
    void patchNonExistingEnquadramento() throws Exception {
        ...
        restEnquadramentoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, enquadramentoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(enquadramentoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEnquadramento() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEnquadramentoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(enquadramentoDTO)))
            .andExpect(status().isMethodNotAllowed());
