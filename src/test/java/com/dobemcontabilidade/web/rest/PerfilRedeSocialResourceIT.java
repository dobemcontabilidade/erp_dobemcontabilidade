...
class PerfilRedeSocialResourceIT {
    ...
    private static final TipoRedeSocialEnum DEFAULT_TIPO_REDE = TipoRedeSocialEnum.FACEBOOK;
    private static final TipoRedeSocialEnum UPDATED_TIPO_REDE = TipoRedeSocialEnum.TWITTER;
    ...
    void createPerfilRedeSocial() throws Exception {
        ...
        PerfilRedeSocialDTO perfilRedeSocialDTO = perfilRedeSocialMapper.toDto(perfilRedeSocial);
        var returnedPerfilRedeSocialDTO = om.readValue(
            restPerfilRedeSocialMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(perfilRedeSocialDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PerfilRedeSocialDTO.class
        );

        // Validate the PerfilRedeSocial in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPerfilRedeSocial = perfilRedeSocialMapper.toEntity(returnedPerfilRedeSocialDTO);
    ...
    void putExistingPerfilRedeSocial() throws Exception {
        ...
        PerfilRedeSocialDTO perfilRedeSocialDTO = perfilRedeSocialMapper.toDto(updatedPerfilRedeSocial);

        restPerfilRedeSocialMockMvc
            .perform(
                put(ENTITY_API_URL_ID, perfilRedeSocialDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(perfilRedeSocialDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPerfilRedeSocial() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPerfilRedeSocialMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(perfilRedeSocialDTO)))
    ...
    void partialUpdatePerfilRedeSocialWithPatch() throws Exception {
        ...
        partialUpdatedPerfilRedeSocial.urlPerfil(UPDATED_URL_PERFIL).tipoRede(UPDATED_TIPO_REDE);
    ...
    void patchNonExistingPerfilRedeSocial() throws Exception {
        ...
        restPerfilRedeSocialMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, perfilRedeSocialDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(perfilRedeSocialDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPerfilRedeSocial() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPerfilRedeSocialMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(perfilRedeSocialDTO)))
            .andExpect(status().isMethodNotAllowed());
