...
class DescontoPlanoContaAzulResourceIT {
    ...
    void createDescontoPlanoContaAzul() throws Exception {
        ...
        var returnedDescontoPlanoContaAzul = om.readValue(
            restDescontoPlanoContaAzulMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(descontoPlanoContaAzul)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            DescontoPlanoContaAzul.class
        );
    ...
    void putExistingDescontoPlanoContaAzul() throws Exception {
        ...
        restDescontoPlanoContaAzulMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedDescontoPlanoContaAzul.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedDescontoPlanoContaAzul))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingDescontoPlanoContaAzul() throws Exception {
        ...
        restDescontoPlanoContaAzulMockMvc
            .perform(
                put(ENTITY_API_URL_ID, descontoPlanoContaAzul.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(descontoPlanoContaAzul))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamDescontoPlanoContaAzul() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDescontoPlanoContaAzulMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(descontoPlanoContaAzul)))
    ...
    void patchNonExistingDescontoPlanoContaAzul() throws Exception {
        ...
        restDescontoPlanoContaAzulMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, descontoPlanoContaAzul.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(descontoPlanoContaAzul))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamDescontoPlanoContaAzul() throws Exception {
        ...
        restDescontoPlanoContaAzulMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(descontoPlanoContaAzul))
            )
            .andExpect(status().isMethodNotAllowed());
