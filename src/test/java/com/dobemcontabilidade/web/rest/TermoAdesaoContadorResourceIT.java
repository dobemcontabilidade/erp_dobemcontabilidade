...
class TermoAdesaoContadorResourceIT {
    ...
    public static TermoAdesaoContador createEntity(EntityManager em) {
        TermoAdesaoContador termoAdesaoContador = new TermoAdesaoContador().dataAdesao(DEFAULT_DATA_ADESAO);
        // Add required entity
        Contador contador;
    ...
    public static TermoAdesaoContador createUpdatedEntity(EntityManager em) {
        TermoAdesaoContador termoAdesaoContador = new TermoAdesaoContador().dataAdesao(UPDATED_DATA_ADESAO);
        // Add required entity
        Contador contador;
        if (TestUtil.findAll(em, Contador.class).isEmpty()) {
    ...
    void createTermoAdesaoContador() throws Exception {
        ...
        TermoAdesaoContadorDTO termoAdesaoContadorDTO = termoAdesaoContadorMapper.toDto(termoAdesaoContador);
        var returnedTermoAdesaoContadorDTO = om.readValue(
            restTermoAdesaoContadorMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(termoAdesaoContadorDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            TermoAdesaoContadorDTO.class
        );

        // Validate the TermoAdesaoContador in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedTermoAdesaoContador = termoAdesaoContadorMapper.toEntity(returnedTermoAdesaoContadorDTO);
    ...
    void putExistingTermoAdesaoContador() throws Exception {
        ...
        updatedTermoAdesaoContador.dataAdesao(UPDATED_DATA_ADESAO);
        TermoAdesaoContadorDTO termoAdesaoContadorDTO = termoAdesaoContadorMapper.toDto(updatedTermoAdesaoContador);

        restTermoAdesaoContadorMockMvc
            .perform(
                put(ENTITY_API_URL_ID, termoAdesaoContadorDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(termoAdesaoContadorDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamTermoAdesaoContador() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTermoAdesaoContadorMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(termoAdesaoContadorDTO)))
    ...
    void patchNonExistingTermoAdesaoContador() throws Exception {
        ...
        restTermoAdesaoContadorMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, termoAdesaoContadorDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(termoAdesaoContadorDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamTermoAdesaoContador() throws Exception {
        ...
        restTermoAdesaoContadorMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(termoAdesaoContadorDTO))
            )
            .andExpect(status().isMethodNotAllowed());
