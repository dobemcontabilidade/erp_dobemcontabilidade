...
class TermoDeAdesaoResourceIT {
    ...
    public static TermoDeAdesao createEntity(EntityManager em) {
        TermoDeAdesao termoDeAdesao = new TermoDeAdesao().titulo(DEFAULT_TITULO).descricao(DEFAULT_DESCRICAO).urlDoc(DEFAULT_URL_DOC);
    ...
    public static TermoDeAdesao createUpdatedEntity(EntityManager em) {
        TermoDeAdesao termoDeAdesao = new TermoDeAdesao().titulo(UPDATED_TITULO).descricao(UPDATED_DESCRICAO).urlDoc(UPDATED_URL_DOC);
        return termoDeAdesao;
    }
    ...
    void createTermoDeAdesao() throws Exception {
        ...
        TermoDeAdesaoDTO termoDeAdesaoDTO = termoDeAdesaoMapper.toDto(termoDeAdesao);
        var returnedTermoDeAdesaoDTO = om.readValue(
            restTermoDeAdesaoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(termoDeAdesaoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            TermoDeAdesaoDTO.class
        );

        // Validate the TermoDeAdesao in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedTermoDeAdesao = termoDeAdesaoMapper.toEntity(returnedTermoDeAdesaoDTO);
    ...
    @Test
    @Transactional
    void getAllTermoDeAdesaosByUrlDocIsEqualToSomething() throws Exception {
        // Initialize the database
        insertedTermoDeAdesao = termoDeAdesaoRepository.saveAndFlush(termoDeAdesao);

        // Get all the termoDeAdesaoList where urlDoc equals to
        defaultTermoDeAdesaoFiltering("urlDoc.equals=" + DEFAULT_URL_DOC, "urlDoc.equals=" + UPDATED_URL_DOC);
    }

    @Test
    @Transactional
    void getAllTermoDeAdesaosByUrlDocIsInShouldWork() throws Exception {
        // Initialize the database
        insertedTermoDeAdesao = termoDeAdesaoRepository.saveAndFlush(termoDeAdesao);

        // Get all the termoDeAdesaoList where urlDoc in
        defaultTermoDeAdesaoFiltering("urlDoc.in=" + DEFAULT_URL_DOC + "," + UPDATED_URL_DOC, "urlDoc.in=" + UPDATED_URL_DOC);
    }

    @Test
    @Transactional
    void getAllTermoDeAdesaosByUrlDocIsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedTermoDeAdesao = termoDeAdesaoRepository.saveAndFlush(termoDeAdesao);

        // Get all the termoDeAdesaoList where urlDoc is not null
        defaultTermoDeAdesaoFiltering("urlDoc.specified=true", "urlDoc.specified=false");
    }

    @Test
    @Transactional
    void getAllTermoDeAdesaosByUrlDocContainsSomething() throws Exception {
        // Initialize the database
        insertedTermoDeAdesao = termoDeAdesaoRepository.saveAndFlush(termoDeAdesao);

        // Get all the termoDeAdesaoList where urlDoc contains
        defaultTermoDeAdesaoFiltering("urlDoc.contains=" + DEFAULT_URL_DOC, "urlDoc.contains=" + UPDATED_URL_DOC);
    }

    @Test
    @Transactional
    void getAllTermoDeAdesaosByUrlDocNotContainsSomething() throws Exception {
        // Initialize the database
        insertedTermoDeAdesao = termoDeAdesaoRepository.saveAndFlush(termoDeAdesao);

        // Get all the termoDeAdesaoList where urlDoc does not contain
        defaultTermoDeAdesaoFiltering("urlDoc.doesNotContain=" + UPDATED_URL_DOC, "urlDoc.doesNotContain=" + DEFAULT_URL_DOC);
    }
    ...
    void putExistingTermoDeAdesao() throws Exception {
        ...
        updatedTermoDeAdesao.titulo(UPDATED_TITULO).descricao(UPDATED_DESCRICAO).urlDoc(UPDATED_URL_DOC);
        TermoDeAdesaoDTO termoDeAdesaoDTO = termoDeAdesaoMapper.toDto(updatedTermoDeAdesao);

        restTermoDeAdesaoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, termoDeAdesaoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(termoDeAdesaoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamTermoDeAdesao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTermoDeAdesaoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(termoDeAdesaoDTO)))
    ...
    void fullUpdateTermoDeAdesaoWithPatch() throws Exception {
        ...
        partialUpdatedTermoDeAdesao.titulo(UPDATED_TITULO).descricao(UPDATED_DESCRICAO).urlDoc(UPDATED_URL_DOC);
    ...
    void patchNonExistingTermoDeAdesao() throws Exception {
        ...
        restTermoDeAdesaoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, termoDeAdesaoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(termoDeAdesaoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamTermoDeAdesao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTermoDeAdesaoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(termoDeAdesaoDTO)))
            .andExpect(status().isMethodNotAllowed());
