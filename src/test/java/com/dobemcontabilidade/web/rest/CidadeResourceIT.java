...
class CidadeResourceIT {
    ...
    void createCidade() throws Exception {
        ...
        var returnedCidade = om.readValue(
            restCidadeMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(cidade)))
    ...
    void putWithMissingIdPathParamCidade() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCidadeMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(cidade)))
    ...
    void partialUpdateCidadeWithPatch() throws Exception {
        ...
        partialUpdatedCidade.abertura(UPDATED_ABERTURA);
    ...
    void patchWithMissingIdPathParamCidade() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCidadeMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(cidade)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteCidade() throws Exception {
        ...
        // Delete the cidade
        restCidadeMockMvc
            .perform(delete(ENTITY_API_URL_ID, cidade.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
