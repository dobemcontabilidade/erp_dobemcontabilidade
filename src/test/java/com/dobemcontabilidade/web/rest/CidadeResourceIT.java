...
class CidadeResourceIT {
    ...
    void createCidade() throws Exception {
        ...
        var returnedCidadeDTO = om.readValue(
            restCidadeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(cidadeDTO)))
        ...
        var returnedCidade = cidadeMapper.toEntity(returnedCidadeDTO);
    ...
    void putExistingCidade() throws Exception {
        ...
        restCidadeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, cidadeDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(cidadeDTO))
            )
            .andExpect(status().isOk());
        ...
        restCidadeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(cidadeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamCidade() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCidadeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(cidadeDTO)))
    ...
    void partialUpdateCidadeWithPatch() throws Exception {
        ...
        partialUpdatedCidade.contratacao(UPDATED_CONTRATACAO);
    ...
    void patchWithMissingIdPathParamCidade() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCidadeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(cidadeDTO)))
            .andExpect(status().isMethodNotAllowed());
