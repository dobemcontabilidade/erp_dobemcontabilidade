...
class CompetenciaResourceIT {
    ...
    void createCompetencia() throws Exception {
        ...
        var returnedCompetenciaDTO = om.readValue(
            restCompetenciaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(competenciaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CompetenciaDTO.class
        );

        // Validate the Competencia in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCompetencia = competenciaMapper.toEntity(returnedCompetenciaDTO);
    ...
    void putExistingCompetencia() throws Exception {
        ...
        restCompetenciaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, competenciaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(competenciaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCompetencia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCompetenciaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(competenciaDTO)))
    ...
    void partialUpdateCompetenciaWithPatch() throws Exception {
        ...
        partialUpdatedCompetencia.numero(UPDATED_NUMERO);
    ...
    void patchNonExistingCompetencia() throws Exception {
        ...
        restCompetenciaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, competenciaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(competenciaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCompetencia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCompetenciaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(competenciaDTO)))
            .andExpect(status().isMethodNotAllowed());
