...
class PagamentoResourceIT {
    ...
    private static final SituacaoPagamentoEnum DEFAULT_SITUACAO = SituacaoPagamentoEnum.A_PAGAR;
    ...
    void createPagamento() throws Exception {
        ...
        var returnedPagamentoDTO = om.readValue(
            restPagamentoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(pagamentoDTO)))
        ...
        var returnedPagamento = pagamentoMapper.toEntity(returnedPagamentoDTO);
        ...
        restPagamentoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, pagamentoDTO.getId())
                    ...
                    .content(om.writeValueAsBytes(pagamentoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPagamento() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPagamentoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(pagamentoDTO)))
    ...
    void patchWithMissingIdPathParamPagamento() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPagamentoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(pagamentoDTO)))
            .andExpect(status().isMethodNotAllowed());
