...
class AdicionalEnquadramentoResourceIT {
    ...
    void createAdicionalEnquadramento() throws Exception {
        ...
        var returnedAdicionalEnquadramento = om.readValue(
            restAdicionalEnquadramentoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalEnquadramento)))
    ...
    void putWithMissingIdPathParamAdicionalEnquadramento() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalEnquadramentoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalEnquadramento)))
    ...
    void patchWithMissingIdPathParamAdicionalEnquadramento() throws Exception {
        ...
        restAdicionalEnquadramentoMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(adicionalEnquadramento))
            )
            .andExpect(status().isMethodNotAllowed());
