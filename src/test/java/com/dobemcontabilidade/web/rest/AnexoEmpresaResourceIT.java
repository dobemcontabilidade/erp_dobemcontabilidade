...
class AnexoEmpresaResourceIT {
    ...
    public static AnexoEmpresa createEntity(EntityManager em) {
        AnexoEmpresa anexoEmpresa = new AnexoEmpresa().urlAnexo(DEFAULT_URL_ANEXO);
        // Add required entity
        Empresa empresa;
        ...
        // Add required entity
        AnexoRequeridoEmpresa anexoRequeridoEmpresa;
        if (TestUtil.findAll(em, AnexoRequeridoEmpresa.class).isEmpty()) {
            anexoRequeridoEmpresa = AnexoRequeridoEmpresaResourceIT.createEntity(em);
            em.persist(anexoRequeridoEmpresa);
            em.flush();
        } else {
            anexoRequeridoEmpresa = TestUtil.findAll(em, AnexoRequeridoEmpresa.class).get(0);
        }
        anexoEmpresa.setAnexoRequeridoEmpresa(anexoRequeridoEmpresa);
        return anexoEmpresa;
    }
    ...
    public static AnexoEmpresa createUpdatedEntity(EntityManager em) {
        AnexoEmpresa anexoEmpresa = new AnexoEmpresa().urlAnexo(UPDATED_URL_ANEXO);
        // Add required entity
        Empresa empresa;
        if (TestUtil.findAll(em, Empresa.class).isEmpty()) {
        ...
        AnexoRequeridoEmpresa anexoRequeridoEmpresa;
        if (TestUtil.findAll(em, AnexoRequeridoEmpresa.class).isEmpty()) {
            anexoRequeridoEmpresa = AnexoRequeridoEmpresaResourceIT.createUpdatedEntity(em);
            em.persist(anexoRequeridoEmpresa);
            em.flush();
        } else {
            anexoRequeridoEmpresa = TestUtil.findAll(em, AnexoRequeridoEmpresa.class).get(0);
    ...
    void createAnexoEmpresa() throws Exception {
        ...
        var returnedAnexoEmpresa = om.readValue(
            restAnexoEmpresaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(anexoEmpresa)))
    ...
    @Test
    @Transactional
    void getAllAnexoEmpresasByAnexoRequeridoEmpresaIsEqualToSomething() throws Exception {
        AnexoRequeridoEmpresa anexoRequeridoEmpresa;
        if (TestUtil.findAll(em, AnexoRequeridoEmpresa.class).isEmpty()) {
            anexoEmpresaRepository.saveAndFlush(anexoEmpresa);
            anexoRequeridoEmpresa = AnexoRequeridoEmpresaResourceIT.createEntity(em);
        ...
        anexoEmpresa.setAnexoRequeridoEmpresa(anexoRequeridoEmpresa);
        anexoEmpresaRepository.saveAndFlush(anexoEmpresa);
        Long anexoRequeridoEmpresaId = anexoRequeridoEmpresa.getId();
        // Get all the anexoEmpresaList where anexoRequeridoEmpresa equals to anexoRequeridoEmpresaId
        defaultAnexoEmpresaShouldBeFound("anexoRequeridoEmpresaId.equals=" + anexoRequeridoEmpresaId);

        // Get all the anexoEmpresaList where anexoRequeridoEmpresa equals to (anexoRequeridoEmpresaId + 1)
        defaultAnexoEmpresaShouldNotBeFound("anexoRequeridoEmpresaId.equals=" + (anexoRequeridoEmpresaId + 1));
    }
    ...
    void putExistingAnexoEmpresa() throws Exception {
        ...
        updatedAnexoEmpresa.urlAnexo(UPDATED_URL_ANEXO);

        restAnexoEmpresaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAnexoEmpresa.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAnexoEmpresa))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAnexoEmpresa() throws Exception {
        ...
        restAnexoEmpresaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, anexoEmpresa.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(anexoEmpresa))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAnexoEmpresa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAnexoEmpresaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(anexoEmpresa)))
    ...
    void patchNonExistingAnexoEmpresa() throws Exception {
        ...
        restAnexoEmpresaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, anexoEmpresa.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(anexoEmpresa))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAnexoEmpresa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAnexoEmpresaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(anexoEmpresa)))
            .andExpect(status().isMethodNotAllowed());
