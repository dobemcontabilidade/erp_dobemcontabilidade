...
class DepartamentoEmpresaResourceIT {
    ...
    void createDepartamentoEmpresa() throws Exception {
        ...
        DepartamentoEmpresaDTO departamentoEmpresaDTO = departamentoEmpresaMapper.toDto(departamentoEmpresa);
        var returnedDepartamentoEmpresaDTO = om.readValue(
            restDepartamentoEmpresaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(departamentoEmpresaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            DepartamentoEmpresaDTO.class
        );

        // Validate the DepartamentoEmpresa in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedDepartamentoEmpresa = departamentoEmpresaMapper.toEntity(returnedDepartamentoEmpresaDTO);
    ...
    void putExistingDepartamentoEmpresa() throws Exception {
        ...
        DepartamentoEmpresaDTO departamentoEmpresaDTO = departamentoEmpresaMapper.toDto(updatedDepartamentoEmpresa);

        restDepartamentoEmpresaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, departamentoEmpresaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(departamentoEmpresaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamDepartamentoEmpresa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDepartamentoEmpresaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(departamentoEmpresaDTO)))
    ...
    void partialUpdateDepartamentoEmpresaWithPatch() throws Exception {
        ...
        partialUpdatedDepartamentoEmpresa.pontuacao(UPDATED_PONTUACAO);
    ...
    void patchNonExistingDepartamentoEmpresa() throws Exception {
        ...
        restDepartamentoEmpresaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, departamentoEmpresaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(departamentoEmpresaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamDepartamentoEmpresa() throws Exception {
        ...
        restDepartamentoEmpresaMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(departamentoEmpresaDTO))
            )
            .andExpect(status().isMethodNotAllowed());
