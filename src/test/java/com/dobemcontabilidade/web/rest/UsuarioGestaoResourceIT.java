...
class UsuarioGestaoResourceIT {
    ...
    private static final SituacaoUsuarioGestaoEnum DEFAULT_SITUACAO_USUARIO_GESTAO = SituacaoUsuarioGestaoEnum.ATIVO;
    private static final SituacaoUsuarioGestaoEnum UPDATED_SITUACAO_USUARIO_GESTAO = SituacaoUsuarioGestaoEnum.INATIVO;
    ...
    public static UsuarioGestao createEntity(EntityManager em) {
        UsuarioGestao usuarioGestao = new UsuarioGestao()
            ...
            .situacaoUsuarioGestao(DEFAULT_SITUACAO_USUARIO_GESTAO);
        // Add required entity
    ...
    public static UsuarioGestao createUpdatedEntity(EntityManager em) {
        UsuarioGestao usuarioGestao = new UsuarioGestao()
            ...
            .situacaoUsuarioGestao(UPDATED_SITUACAO_USUARIO_GESTAO);
        // Add required entity
        Administrador administrador;
        if (TestUtil.findAll(em, Administrador.class).isEmpty()) {
    ...
    void createUsuarioGestao() throws Exception {
        ...
        var returnedUsuarioGestao = om.readValue(
            restUsuarioGestaoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(usuarioGestao)))
    ...
    void getAllUsuarioGestaos() throws Exception {
        ...
        restUsuarioGestaoMockMvc
            ...
            .andExpect(jsonPath("$.[*].situacaoUsuarioGestao").value(hasItem(DEFAULT_SITUACAO_USUARIO_GESTAO.toString())));
    }
    ...
    void getUsuarioGestao() throws Exception {
        ...
        restUsuarioGestaoMockMvc
            ...
            .andExpect(jsonPath("$.situacaoUsuarioGestao").value(DEFAULT_SITUACAO_USUARIO_GESTAO.toString()));
    }
    ...
    @Test
    @Transactional
    void getAllUsuarioGestaosByAtivoIsEqualToSomething() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where ativo equals to
        defaultUsuarioGestaoFiltering("ativo.equals=" + DEFAULT_ATIVO, "ativo.equals=" + UPDATED_ATIVO);
    }

    @Test
    @Transactional
    void getAllUsuarioGestaosByAtivoIsInShouldWork() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where ativo in
        defaultUsuarioGestaoFiltering("ativo.in=" + DEFAULT_ATIVO + "," + UPDATED_ATIVO, "ativo.in=" + UPDATED_ATIVO);
    }

    @Test
    @Transactional
    void getAllUsuarioGestaosByAtivoIsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where ativo is not null
        defaultUsuarioGestaoFiltering("ativo.specified=true", "ativo.specified=false");
    }
    ...
    @Test
    @Transactional
    void getAllUsuarioGestaosBySituacaoUsuarioGestaoIsEqualToSomething() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where situacaoUsuarioGestao equals to
        defaultUsuarioGestaoFiltering(
            "situacaoUsuarioGestao.equals=" + DEFAULT_SITUACAO_USUARIO_GESTAO,
            "situacaoUsuarioGestao.equals=" + UPDATED_SITUACAO_USUARIO_GESTAO
        );
    }

    @Test
    @Transactional
    void getAllUsuarioGestaosBySituacaoUsuarioGestaoIsInShouldWork() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where situacaoUsuarioGestao in
        defaultUsuarioGestaoFiltering(
            "situacaoUsuarioGestao.in=" + DEFAULT_SITUACAO_USUARIO_GESTAO + "," + UPDATED_SITUACAO_USUARIO_GESTAO,
            "situacaoUsuarioGestao.in=" + UPDATED_SITUACAO_USUARIO_GESTAO
        );
    }

    @Test
    @Transactional
    void getAllUsuarioGestaosBySituacaoUsuarioGestaoIsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedUsuarioGestao = usuarioGestaoRepository.saveAndFlush(usuarioGestao);

        // Get all the usuarioGestaoList where situacaoUsuarioGestao is not null
        defaultUsuarioGestaoFiltering("situacaoUsuarioGestao.specified=true", "situacaoUsuarioGestao.specified=false");
    }
    ...
    void putExistingUsuarioGestao() throws Exception {
        ...
        restUsuarioGestaoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedUsuarioGestao.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedUsuarioGestao))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingUsuarioGestao() throws Exception {
        ...
        restUsuarioGestaoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, usuarioGestao.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(usuarioGestao))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamUsuarioGestao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUsuarioGestaoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(usuarioGestao)))
    ...
    void partialUpdateUsuarioGestaoWithPatch() throws Exception {
        ...
        partialUpdatedUsuarioGestao.senha(UPDATED_SENHA);
    ...
    void patchNonExistingUsuarioGestao() throws Exception {
        ...
        restUsuarioGestaoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, usuarioGestao.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(usuarioGestao))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamUsuarioGestao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUsuarioGestaoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(usuarioGestao)))
            .andExpect(status().isMethodNotAllowed());
