...
class PlanoContabilResourceIT {
    ...
    void createPlanoContabil() throws Exception {
        ...
        var returnedPlanoContabil = om.readValue(
            restPlanoContabilMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(planoContabil)))
    ...
    void putExistingPlanoContabil() throws Exception {
        ...
        restPlanoContabilMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedPlanoContabil.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedPlanoContabil))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingPlanoContabil() throws Exception {
        ...
        restPlanoContabilMockMvc
            .perform(
                put(ENTITY_API_URL_ID, planoContabil.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(planoContabil))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPlanoContabil() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPlanoContabilMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(planoContabil)))
    ...
    void patchNonExistingPlanoContabil() throws Exception {
        ...
        restPlanoContabilMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, planoContabil.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(planoContabil))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPlanoContabil() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPlanoContabilMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(planoContabil)))
            .andExpect(status().isMethodNotAllowed());
