...
class DivisaoCnaeResourceIT {
    ...
    void createDivisaoCnae() throws Exception {
        ...
        var returnedDivisaoCnae = om.readValue(
            restDivisaoCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(divisaoCnae)))
    ...
    void putExistingDivisaoCnae() throws Exception {
        ...
        restDivisaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedDivisaoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedDivisaoCnae))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingDivisaoCnae() throws Exception {
        ...
        restDivisaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, divisaoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(divisaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamDivisaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDivisaoCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(divisaoCnae)))
    ...
    void patchNonExistingDivisaoCnae() throws Exception {
        ...
        restDivisaoCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, divisaoCnae.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(divisaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamDivisaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDivisaoCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(divisaoCnae)))
            .andExpect(status().isMethodNotAllowed());
