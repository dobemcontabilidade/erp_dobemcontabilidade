...
class TipoDenunciaResourceIT {
    ...
    void createTipoDenuncia() throws Exception {
        ...
        var returnedTipoDenunciaDTO = om.readValue(
            restTipoDenunciaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tipoDenunciaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            TipoDenunciaDTO.class
        );

        // Validate the TipoDenuncia in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedTipoDenuncia = tipoDenunciaMapper.toEntity(returnedTipoDenunciaDTO);
    ...
    void putExistingTipoDenuncia() throws Exception {
        ...
        restTipoDenunciaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, tipoDenunciaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(tipoDenunciaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamTipoDenuncia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTipoDenunciaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tipoDenunciaDTO)))
    ...
    void patchNonExistingTipoDenuncia() throws Exception {
        ...
        restTipoDenunciaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, tipoDenunciaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(tipoDenunciaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamTipoDenuncia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTipoDenunciaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(tipoDenunciaDTO)))
            .andExpect(status().isMethodNotAllowed());
