...
class BancoResourceIT {
    ...
    void createBanco() throws Exception {
        ...
        // Create the Banco
        var returnedBanco = om.readValue(
            restBancoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(banco)))
        ...
        restBancoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedBanco.getId())
                    ...
                    .content(om.writeValueAsBytes(updatedBanco))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingBanco() throws Exception {
        ...
        restBancoMockMvc
            .perform(put(ENTITY_API_URL_ID, banco.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(banco)))
            .andExpect(status().isBadRequest());
        ...
        restBancoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(banco))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamBanco() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBancoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(banco)))
    ...
    void patchNonExistingBanco() throws Exception {
        ...
        restBancoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, banco.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(banco))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamBanco() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBancoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(banco)))
            .andExpect(status().isMethodNotAllowed());
