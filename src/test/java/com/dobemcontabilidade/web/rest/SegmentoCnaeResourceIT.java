...
class SegmentoCnaeResourceIT {
    ...
    void createSegmentoCnae() throws Exception {
        ...
        var returnedSegmentoCnae = om.readValue(
            restSegmentoCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(segmentoCnae)))
    ...
    @Test
    @Transactional
    void getAllSegmentoCnaesByEmpresaModeloIsEqualToSomething() throws Exception {
        ...
        em.flush();
        segmentoCnae.addEmpresaModelo(empresaModelo);
        segmentoCnaeRepository.saveAndFlush(segmentoCnae);
        Long empresaModeloId = empresaModelo.getId();
        // Get all the segmentoCnaeList where empresaModelo equals to empresaModeloId
        defaultSegmentoCnaeShouldBeFound("empresaModeloId.equals=" + empresaModeloId);

        // Get all the segmentoCnaeList where empresaModelo equals to (empresaModeloId + 1)
        defaultSegmentoCnaeShouldNotBeFound("empresaModeloId.equals=" + (empresaModeloId + 1));
    }
    ...
    void putExistingSegmentoCnae() throws Exception {
        ...
        restSegmentoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedSegmentoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedSegmentoCnae))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingSegmentoCnae() throws Exception {
        ...
        restSegmentoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, segmentoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(segmentoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamSegmentoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSegmentoCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(segmentoCnae)))
    ...
    void partialUpdateSegmentoCnaeWithPatch() throws Exception {
        ...
        partialUpdatedSegmentoCnae.nome(UPDATED_NOME).descricao(UPDATED_DESCRICAO).icon(UPDATED_ICON).tags(UPDATED_TAGS).tipo(UPDATED_TIPO);
    ...
    void patchNonExistingSegmentoCnae() throws Exception {
        ...
        restSegmentoCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, segmentoCnae.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(segmentoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSegmentoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSegmentoCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(segmentoCnae)))
            .andExpect(status().isMethodNotAllowed());
