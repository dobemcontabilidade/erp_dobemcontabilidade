...
class EstadoResourceIT {
    ...
    void createEstado() throws Exception {
        ...
        var returnedEstadoDTO = om.readValue(
            restEstadoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(estadoDTO)))
        ...
        var returnedEstado = estadoMapper.toEntity(returnedEstadoDTO);
    ...
    void putExistingEstado() throws Exception {
        ...
        restEstadoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, estadoDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(estadoDTO))
            )
            .andExpect(status().isOk());
        ...
        restEstadoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(estadoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamEstado() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEstadoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(estadoDTO)))
    ...
    void patchWithMissingIdPathParamEstado() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEstadoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(estadoDTO)))
            .andExpect(status().isMethodNotAllowed());
