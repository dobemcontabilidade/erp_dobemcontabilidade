...
class SubclasseCnaeResourceIT {
    ...
    void createSubclasseCnae() throws Exception {
        ...
        var returnedSubclasseCnae = om.readValue(
            restSubclasseCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(subclasseCnae)))
    ...
    void putExistingSubclasseCnae() throws Exception {
        ...
        restSubclasseCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedSubclasseCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedSubclasseCnae))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingSubclasseCnae() throws Exception {
        ...
        restSubclasseCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, subclasseCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(subclasseCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamSubclasseCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSubclasseCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(subclasseCnae)))
    ...
    void partialUpdateSubclasseCnaeWithPatch() throws Exception {
        ...
        partialUpdatedSubclasseCnae.descricao(UPDATED_DESCRICAO).atendidoFreemium(UPDATED_ATENDIDO_FREEMIUM).aceitaMEI(UPDATED_ACEITA_MEI);
    ...
    void patchNonExistingSubclasseCnae() throws Exception {
        ...
        restSubclasseCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, subclasseCnae.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(subclasseCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSubclasseCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSubclasseCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(subclasseCnae)))
            .andExpect(status().isMethodNotAllowed());
