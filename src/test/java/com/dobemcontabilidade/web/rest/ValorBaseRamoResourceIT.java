...
class ValorBaseRamoResourceIT {
    ...
    void createValorBaseRamo() throws Exception {
        ...
        var returnedValorBaseRamo = om.readValue(
            restValorBaseRamoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(valorBaseRamo)))
    ...
    void putExistingValorBaseRamo() throws Exception {
        ...
        restValorBaseRamoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedValorBaseRamo.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedValorBaseRamo))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingValorBaseRamo() throws Exception {
        ...
        restValorBaseRamoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, valorBaseRamo.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(valorBaseRamo))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamValorBaseRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restValorBaseRamoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(valorBaseRamo)))
    ...
    void patchNonExistingValorBaseRamo() throws Exception {
        ...
        restValorBaseRamoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, valorBaseRamo.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(valorBaseRamo))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamValorBaseRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restValorBaseRamoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(valorBaseRamo)))
            .andExpect(status().isMethodNotAllowed());
