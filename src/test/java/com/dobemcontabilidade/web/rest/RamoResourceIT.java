...
class RamoResourceIT {
    ...
    void createRamo() throws Exception {
        ...
        // Create the Ramo
        RamoDTO ramoDTO = ramoMapper.toDto(ramo);
        var returnedRamoDTO = om.readValue(
            restRamoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(ramoDTO)))
        ...
        var returnedRamo = ramoMapper.toEntity(returnedRamoDTO);
    ...
    void putExistingRamo() throws Exception {
        ...
        RamoDTO ramoDTO = ramoMapper.toDto(updatedRamo);

        restRamoMockMvc
            .perform(put(ENTITY_API_URL_ID, ramoDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(ramoDTO)))
            .andExpect(status().isOk());
        ...
        restRamoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(ramoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRamoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(ramoDTO)))
    ...
    void patchNonExistingRamo() throws Exception {
        ...
        restRamoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, ramoDTO.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(ramoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRamoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(ramoDTO)))
            .andExpect(status().isMethodNotAllowed());
