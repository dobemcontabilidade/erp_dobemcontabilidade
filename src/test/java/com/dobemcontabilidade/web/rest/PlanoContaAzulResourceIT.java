...
class PlanoContaAzulResourceIT {
    ...
    private static final SituacaoPlanoContaAzulEnum DEFAULT_SITUACAO = SituacaoPlanoContaAzulEnum.ATIVO;
    private static final SituacaoPlanoContaAzulEnum UPDATED_SITUACAO = SituacaoPlanoContaAzulEnum.INATIVO;
    ...
    void createPlanoContaAzul() throws Exception {
        ...
        PlanoContaAzulDTO planoContaAzulDTO = planoContaAzulMapper.toDto(planoContaAzul);
        var returnedPlanoContaAzulDTO = om.readValue(
            restPlanoContaAzulMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(planoContaAzulDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PlanoContaAzulDTO.class
        );

        // Validate the PlanoContaAzul in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPlanoContaAzul = planoContaAzulMapper.toEntity(returnedPlanoContaAzulDTO);
    ...
    void putExistingPlanoContaAzul() throws Exception {
        ...
        updatedPlanoContaAzul
            ...
            .suporte(UPDATED_SUPORTE)
            .situacao(UPDATED_SITUACAO);
        PlanoContaAzulDTO planoContaAzulDTO = planoContaAzulMapper.toDto(updatedPlanoContaAzul);

        restPlanoContaAzulMockMvc
            .perform(
                put(ENTITY_API_URL_ID, planoContaAzulDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(planoContaAzulDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPlanoContaAzul() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPlanoContaAzulMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(planoContaAzulDTO)))
    ...
    void patchNonExistingPlanoContaAzul() throws Exception {
        ...
        restPlanoContaAzulMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, planoContaAzulDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(planoContaAzulDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPlanoContaAzul() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPlanoContaAzulMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(planoContaAzulDTO)))
            .andExpect(status().isMethodNotAllowed());
