...
class SocioResourceIT {
    ...
    private static final FuncaoSocioEnum UPDATED_FUNCAO_SOCIO = FuncaoSocioEnum.SOCIOADMINISTRADOR;
    ...
    public static Socio createEntity(EntityManager em) {
        ...
        socio.setUsuarioEmpresa(usuarioEmpresa);
        // Add required entity
        Empresa empresa;
    ...
    void createSocio() throws Exception {
        ...
        var returnedSocio = om.readValue(
            restSocioMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(socio)))
    ...
    @Test
    @Transactional
    void getAllSociosByUsuarioEmpresaIsEqualToSomething() throws Exception {
        // Get already existing entity
        UsuarioEmpresa usuarioEmpresa = socio.getUsuarioEmpresa();
        socioRepository.saveAndFlush(socio);
        Long usuarioEmpresaId = usuarioEmpresa.getId();
        // Get all the socioList where usuarioEmpresa equals to usuarioEmpresaId
        defaultSocioShouldBeFound("usuarioEmpresaId.equals=" + usuarioEmpresaId);

        // Get all the socioList where usuarioEmpresa equals to (usuarioEmpresaId + 1)
        defaultSocioShouldNotBeFound("usuarioEmpresaId.equals=" + (usuarioEmpresaId + 1));
    }
    ...
    @Test
    @Transactional
    void getAllSociosByProfissaoIsEqualToSomething() throws Exception {
        Profissao profissao;
        ...
        // Get all the socioList where profissao equals to profissaoId
        defaultSocioShouldBeFound("profissaoId.equals=" + profissaoId);

        // Get all the socioList where profissao equals to (profissaoId + 1)
        defaultSocioShouldNotBeFound("profissaoId.equals=" + (profissaoId + 1));
    }
    ...
    void putNonExistingSocio() throws Exception {
        ...
        restSocioMockMvc
            .perform(put(ENTITY_API_URL_ID, socio.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(socio)))
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamSocio() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSocioMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(socio)))
    ...
    void patchNonExistingSocio() throws Exception {
        ...
        restSocioMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, socio.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(socio))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSocio() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSocioMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(socio)))
            .andExpect(status().isMethodNotAllowed());
