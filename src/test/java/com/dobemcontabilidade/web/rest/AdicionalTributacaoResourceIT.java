...
class AdicionalTributacaoResourceIT {
    ...
    public static AdicionalTributacao createEntity(EntityManager em) {
        ...
        adicionalTributacao.setPlanoContabil(planoContabil);
    ...
    void createAdicionalTributacao() throws Exception {
        ...
        AdicionalTributacaoDTO adicionalTributacaoDTO = adicionalTributacaoMapper.toDto(adicionalTributacao);
        var returnedAdicionalTributacaoDTO = om.readValue(
            restAdicionalTributacaoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalTributacaoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AdicionalTributacaoDTO.class
        );

        // Validate the AdicionalTributacao in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAdicionalTributacao = adicionalTributacaoMapper.toEntity(returnedAdicionalTributacaoDTO);
    ...
    @Test
    @Transactional
    void getAllAdicionalTributacaosByPlanoContabilIsEqualToSomething() throws Exception {
        PlanoContabil planoContabil;
        if (TestUtil.findAll(em, PlanoContabil.class).isEmpty()) {
            adicionalTributacaoRepository.saveAndFlush(adicionalTributacao);
            planoContabil = PlanoContabilResourceIT.createEntity(em);
        ...
        // Get all the adicionalTributacaoList where planoContabil equals to planoContabilId
        defaultAdicionalTributacaoShouldBeFound("planoContabilId.equals=" + planoContabilId);

        // Get all the adicionalTributacaoList where planoContabil equals to (planoContabilId + 1)
        defaultAdicionalTributacaoShouldNotBeFound("planoContabilId.equals=" + (planoContabilId + 1));
    }
    ...
    void putExistingAdicionalTributacao() throws Exception {
        ...
        AdicionalTributacaoDTO adicionalTributacaoDTO = adicionalTributacaoMapper.toDto(updatedAdicionalTributacao);

        restAdicionalTributacaoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, adicionalTributacaoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(adicionalTributacaoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAdicionalTributacao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalTributacaoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalTributacaoDTO)))
    ...
    void patchNonExistingAdicionalTributacao() throws Exception {
        ...
        restAdicionalTributacaoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, adicionalTributacaoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(adicionalTributacaoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAdicionalTributacao() throws Exception {
        ...
        restAdicionalTributacaoMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(adicionalTributacaoDTO))
            )
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteAdicionalTributacao() throws Exception {
        ...
        restAdicionalTributacaoMockMvc
            .perform(delete(ENTITY_API_URL_ID, adicionalTributacao.getId()).accept(MediaType.APPLICATION_JSON))
