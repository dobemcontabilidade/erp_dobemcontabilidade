...
class AdicionalRamoResourceIT {
    ...
    public static AdicionalRamo createEntity(EntityManager em) {
        ...
        adicionalRamo.setPlanoContabil(planoContabil);
    ...
    void createAdicionalRamo() throws Exception {
        ...
        AdicionalRamoDTO adicionalRamoDTO = adicionalRamoMapper.toDto(adicionalRamo);
        var returnedAdicionalRamoDTO = om.readValue(
            restAdicionalRamoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalRamoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AdicionalRamoDTO.class
        );

        // Validate the AdicionalRamo in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAdicionalRamo = adicionalRamoMapper.toEntity(returnedAdicionalRamoDTO);
    ...
    @Test
    @Transactional
    void getAllAdicionalRamosByPlanoContabilIsEqualToSomething() throws Exception {
        PlanoContabil planoContabil;
        if (TestUtil.findAll(em, PlanoContabil.class).isEmpty()) {
            adicionalRamoRepository.saveAndFlush(adicionalRamo);
            planoContabil = PlanoContabilResourceIT.createEntity(em);
        ...
        // Get all the adicionalRamoList where planoContabil equals to planoContabilId
        defaultAdicionalRamoShouldBeFound("planoContabilId.equals=" + planoContabilId);

        // Get all the adicionalRamoList where planoContabil equals to (planoContabilId + 1)
        defaultAdicionalRamoShouldNotBeFound("planoContabilId.equals=" + (planoContabilId + 1));
    }
    ...
    void putExistingAdicionalRamo() throws Exception {
        ...
        AdicionalRamoDTO adicionalRamoDTO = adicionalRamoMapper.toDto(updatedAdicionalRamo);

        restAdicionalRamoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, adicionalRamoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(adicionalRamoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAdicionalRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalRamoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalRamoDTO)))
    ...
    void patchNonExistingAdicionalRamo() throws Exception {
        ...
        restAdicionalRamoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, adicionalRamoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(adicionalRamoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAdicionalRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalRamoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(adicionalRamoDTO)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteAdicionalRamo() throws Exception {
        ...
        restAdicionalRamoMockMvc
            .perform(delete(ENTITY_API_URL_ID, adicionalRamo.getId()).accept(MediaType.APPLICATION_JSON))
