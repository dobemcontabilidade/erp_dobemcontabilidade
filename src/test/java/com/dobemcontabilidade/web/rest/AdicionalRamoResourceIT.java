...
class AdicionalRamoResourceIT {
    ...
    void createAdicionalRamo() throws Exception {
        ...
        var returnedAdicionalRamo = om.readValue(
            restAdicionalRamoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalRamo)))
    ...
    void putWithMissingIdPathParamAdicionalRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalRamoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adicionalRamo)))
    ...
    void patchWithMissingIdPathParamAdicionalRamo() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdicionalRamoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(adicionalRamo)))
            .andExpect(status().isMethodNotAllowed());
