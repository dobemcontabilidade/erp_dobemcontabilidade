...
class SubtarefaResourceIT {
    ...
    void createSubtarefa() throws Exception {
        ...
        SubtarefaDTO subtarefaDTO = subtarefaMapper.toDto(subtarefa);
        var returnedSubtarefaDTO = om.readValue(
            restSubtarefaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(subtarefaDTO)))
        ...
        var returnedSubtarefa = subtarefaMapper.toEntity(returnedSubtarefaDTO);
    ...
    void putExistingSubtarefa() throws Exception {
        ...
        SubtarefaDTO subtarefaDTO = subtarefaMapper.toDto(updatedSubtarefa);

        restSubtarefaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, subtarefaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(subtarefaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamSubtarefa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSubtarefaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(subtarefaDTO)))
    ...
    void partialUpdateSubtarefaWithPatch() throws Exception {
        ...
        partialUpdatedSubtarefa.ordem(UPDATED_ORDEM).item(UPDATED_ITEM);
    ...
    void patchNonExistingSubtarefa() throws Exception {
        ...
        restSubtarefaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, subtarefaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(subtarefaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSubtarefa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSubtarefaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(subtarefaDTO)))
            .andExpect(status().isMethodNotAllowed());
