...
class GrupoCnaeResourceIT {
    ...
    void createGrupoCnae() throws Exception {
        ...
        GrupoCnaeDTO grupoCnaeDTO = grupoCnaeMapper.toDto(grupoCnae);
        var returnedGrupoCnaeDTO = om.readValue(
            restGrupoCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(grupoCnaeDTO)))
        ...
        var returnedGrupoCnae = grupoCnaeMapper.toEntity(returnedGrupoCnaeDTO);
    ...
    void putExistingGrupoCnae() throws Exception {
        ...
        GrupoCnaeDTO grupoCnaeDTO = grupoCnaeMapper.toDto(updatedGrupoCnae);

        restGrupoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, grupoCnaeDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(grupoCnaeDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamGrupoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restGrupoCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(grupoCnaeDTO)))
    ...
    void partialUpdateGrupoCnaeWithPatch() throws Exception {
        ...
        partialUpdatedGrupoCnae.codigo(UPDATED_CODIGO);
    ...
    void patchNonExistingGrupoCnae() throws Exception {
        ...
        restGrupoCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, grupoCnaeDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(grupoCnaeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamGrupoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restGrupoCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(grupoCnaeDTO)))
            .andExpect(status().isMethodNotAllowed());
