...
class ObservacaoCnaeResourceIT {
    ...
    void createObservacaoCnae() throws Exception {
        ...
        var returnedObservacaoCnae = om.readValue(
            restObservacaoCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(observacaoCnae)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ObservacaoCnae.class
        );
    ...
    void putExistingObservacaoCnae() throws Exception {
        ...
        restObservacaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedObservacaoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedObservacaoCnae))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingObservacaoCnae() throws Exception {
        ...
        restObservacaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, observacaoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(observacaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamObservacaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restObservacaoCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(observacaoCnae)))
    ...
    void patchNonExistingObservacaoCnae() throws Exception {
        ...
        restObservacaoCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, observacaoCnae.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(observacaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamObservacaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restObservacaoCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(observacaoCnae)))
            .andExpect(status().isMethodNotAllowed());
