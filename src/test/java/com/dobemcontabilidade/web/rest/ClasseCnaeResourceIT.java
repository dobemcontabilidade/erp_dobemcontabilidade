...
class ClasseCnaeResourceIT {
    ...
    void createClasseCnae() throws Exception {
        ...
        ClasseCnaeDTO classeCnaeDTO = classeCnaeMapper.toDto(classeCnae);
        var returnedClasseCnaeDTO = om.readValue(
            restClasseCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(classeCnaeDTO)))
        ...
        var returnedClasseCnae = classeCnaeMapper.toEntity(returnedClasseCnaeDTO);
    ...
    void putExistingClasseCnae() throws Exception {
        ...
        ClasseCnaeDTO classeCnaeDTO = classeCnaeMapper.toDto(updatedClasseCnae);

        restClasseCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, classeCnaeDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(classeCnaeDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamClasseCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restClasseCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(classeCnaeDTO)))
    ...
    void patchNonExistingClasseCnae() throws Exception {
        ...
        restClasseCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, classeCnaeDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(classeCnaeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamClasseCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restClasseCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(classeCnaeDTO)))
            .andExpect(status().isMethodNotAllowed());
