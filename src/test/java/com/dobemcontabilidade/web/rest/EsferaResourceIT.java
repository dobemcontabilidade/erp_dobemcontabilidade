...
class EsferaResourceIT {
    ...
    void createEsfera() throws Exception {
        ...
        // Create the Esfera
        var returnedEsfera = om.readValue(
            restEsferaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(esfera)))
    ...
    void putExistingEsfera() throws Exception {
        ...
        restEsferaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedEsfera.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedEsfera))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingEsfera() throws Exception {
        ...
        restEsferaMockMvc
            .perform(put(ENTITY_API_URL_ID, esfera.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(esfera)))
            .andExpect(status().isBadRequest());
        ...
        restEsferaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(esfera))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamEsfera() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEsferaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(esfera)))
    ...
    void patchNonExistingEsfera() throws Exception {
        ...
        restEsferaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, esfera.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(esfera))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEsfera() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEsferaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(esfera)))
            .andExpect(status().isMethodNotAllowed());
