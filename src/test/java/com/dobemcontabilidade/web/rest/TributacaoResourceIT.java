...
class TributacaoResourceIT {
    ...
    void createTributacao() throws Exception {
        ...
        var returnedTributacao = om.readValue(
            restTributacaoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tributacao)))
    ...
    void putNonExistingTributacao() throws Exception {
        ...
        restTributacaoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, tributacao.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tributacao))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamTributacao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTributacaoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tributacao)))
    ...
    void patchWithMissingIdPathParamTributacao() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTributacaoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(tributacao)))
            .andExpect(status().isMethodNotAllowed());
