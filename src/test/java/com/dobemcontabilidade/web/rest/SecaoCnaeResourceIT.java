...
class SecaoCnaeResourceIT {
    ...
    void createSecaoCnae() throws Exception {
        ...
        var returnedSecaoCnae = om.readValue(
            restSecaoCnaeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(secaoCnae)))
    ...
    void putExistingSecaoCnae() throws Exception {
        ...
        restSecaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedSecaoCnae.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedSecaoCnae))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingSecaoCnae() throws Exception {
        ...
        restSecaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, secaoCnae.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(secaoCnae))
            )
            .andExpect(status().isBadRequest());
        ...
        restSecaoCnaeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(secaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamSecaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSecaoCnaeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(secaoCnae)))
    ...
    void partialUpdateSecaoCnaeWithPatch() throws Exception {
        ...
        partialUpdatedSecaoCnae.codigo(UPDATED_CODIGO);
    ...
    void patchNonExistingSecaoCnae() throws Exception {
        ...
        restSecaoCnaeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, secaoCnae.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(secaoCnae))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSecaoCnae() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSecaoCnaeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(secaoCnae)))
            .andExpect(status().isMethodNotAllowed());
