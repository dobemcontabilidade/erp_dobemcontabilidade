...
class TarefaResourceIT {
    ...
    private static final TipoTarefaEnum UPDATED_TIPO_TAREFA = TipoTarefaEnum.ORDEM_SERVICO;
    ...
    void createTarefa() throws Exception {
        ...
        var returnedTarefaDTO = om.readValue(
            restTarefaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tarefaDTO)))
        ...
        var returnedTarefa = tarefaMapper.toEntity(returnedTarefaDTO);
    ...
    void putExistingTarefa() throws Exception {
        ...
        restTarefaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, tarefaDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tarefaDTO))
            )
            .andExpect(status().isOk());
        ...
        restTarefaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(tarefaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamTarefa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTarefaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(tarefaDTO)))
    ...
    void patchWithMissingIdPathParamTarefa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTarefaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(tarefaDTO)))
            .andExpect(status().isMethodNotAllowed());
