...
class AtividadeEmpresaResourceIT {
    ...
    public static AtividadeEmpresa createEntity(EntityManager em) {
        ...
        atividadeEmpresa.setCnae(subclasseCnae);
        // Add required entity
        Empresa empresa;
    ...
    void createAtividadeEmpresa() throws Exception {
        ...
        var returnedAtividadeEmpresa = om.readValue(
            restAtividadeEmpresaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(atividadeEmpresa)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AtividadeEmpresa.class
        );
    ...
    void putExistingAtividadeEmpresa() throws Exception {
        ...
        restAtividadeEmpresaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAtividadeEmpresa.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAtividadeEmpresa))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAtividadeEmpresa() throws Exception {
        ...
        restAtividadeEmpresaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, atividadeEmpresa.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(atividadeEmpresa))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAtividadeEmpresa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAtividadeEmpresaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(atividadeEmpresa)))
    ...
    void partialUpdateAtividadeEmpresaWithPatch() throws Exception {
        ...
        partialUpdatedAtividadeEmpresa.ordem(UPDATED_ORDEM);
    ...
    void patchNonExistingAtividadeEmpresa() throws Exception {
        ...
        restAtividadeEmpresaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, atividadeEmpresa.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(atividadeEmpresa))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAtividadeEmpresa() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAtividadeEmpresaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(atividadeEmpresa)))
            .andExpect(status().isMethodNotAllowed());
