...
class DenunciaResourceIT {
    ...
    void createDenuncia() throws Exception {
        ...
        var returnedDenunciaDTO = om.readValue(
            restDenunciaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(denunciaDTO)))
        ...
        var returnedDenuncia = denunciaMapper.toEntity(returnedDenunciaDTO);
    ...
    void putExistingDenuncia() throws Exception {
        ...
        restDenunciaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, denunciaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(denunciaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamDenuncia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDenunciaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(denunciaDTO)))
    ...
    void partialUpdateDenunciaWithPatch() throws Exception {
        ...
        partialUpdatedDenuncia.titulo(UPDATED_TITULO).descricao(UPDATED_DESCRICAO);
    ...
    void patchNonExistingDenuncia() throws Exception {
        ...
        restDenunciaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, denunciaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(denunciaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamDenuncia() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restDenunciaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(denunciaDTO)))
            .andExpect(status().isMethodNotAllowed());
