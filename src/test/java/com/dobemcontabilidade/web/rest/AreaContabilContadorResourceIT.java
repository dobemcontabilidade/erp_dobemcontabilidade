...
class AreaContabilContadorResourceIT {
    ...
    void createAreaContabilContador() throws Exception {
        ...
        var returnedAreaContabilContador = om.readValue(
            restAreaContabilContadorMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(areaContabilContador)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AreaContabilContador.class
        );
    ...
    void putExistingAreaContabilContador() throws Exception {
        ...
        restAreaContabilContadorMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAreaContabilContador.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAreaContabilContador))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAreaContabilContador() throws Exception {
        ...
        restAreaContabilContadorMockMvc
            .perform(
                put(ENTITY_API_URL_ID, areaContabilContador.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(areaContabilContador))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAreaContabilContador() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAreaContabilContadorMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(areaContabilContador)))
    ...
    void partialUpdateAreaContabilContadorWithPatch() throws Exception {
        ...
        partialUpdatedAreaContabilContador.ativo(UPDATED_ATIVO);
    ...
    void patchNonExistingAreaContabilContador() throws Exception {
        ...
        restAreaContabilContadorMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, areaContabilContador.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(areaContabilContador))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAreaContabilContador() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAreaContabilContadorMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(areaContabilContador)))
            .andExpect(status().isMethodNotAllowed());
