...
class TarefaTest {
    ...
    @Test
    void documentoTarefaTest() {
        Tarefa tarefa = getTarefaRandomSampleGenerator();
        DocumentoTarefa documentoTarefaBack = getDocumentoTarefaRandomSampleGenerator();

        tarefa.addDocumentoTarefa(documentoTarefaBack);
        assertThat(tarefa.getDocumentoTarefas()).containsOnly(documentoTarefaBack);
        assertThat(documentoTarefaBack.getTarefa()).isEqualTo(tarefa);

        tarefa.removeDocumentoTarefa(documentoTarefaBack);
        assertThat(tarefa.getDocumentoTarefas()).doesNotContain(documentoTarefaBack);
        assertThat(documentoTarefaBack.getTarefa()).isNull();

        tarefa.documentoTarefas(new HashSet<>(Set.of(documentoTarefaBack)));
        assertThat(tarefa.getDocumentoTarefas()).containsOnly(documentoTarefaBack);
        assertThat(documentoTarefaBack.getTarefa()).isEqualTo(tarefa);

        tarefa.setDocumentoTarefas(new HashSet<>());
        assertThat(tarefa.getDocumentoTarefas()).doesNotContain(documentoTarefaBack);
        assertThat(documentoTarefaBack.getTarefa()).isNull();
    }
