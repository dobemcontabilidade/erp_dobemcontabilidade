import { TestBed } from '@angular/core/testing';
import { provideHttpClientTesting, HttpTestingController } from '@angular/common/http/testing';
import { provideHttpClient } from '@angular/common/http';

import { IFuncionario } from '../funcionario.model';
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../funcionario.test-samples';

import { FuncionarioService } from './funcionario.service';

const requireRestSample: IFuncionario = {
  ...sampleWithRequiredData,
};

describe('Funcionario Service', () => {
  let service: FuncionarioService;
  let httpMock: HttpTestingController;
  let expectedResult: IFuncionario | IFuncionario[] | boolean | null;

  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideHttpClient(), provideHttpClientTesting()],
    });
    expectedResult = null;
    service = TestBed.inject(FuncionarioService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  describe('Service methods', () => {
    it('should find an element', () => {
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.find(123).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should create a Funcionario', () => {
      const funcionario = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(funcionario).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'POST' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should update a Funcionario', () => {
      const funcionario = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(funcionario).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PUT' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should partial update a Funcionario', () => {
      const patchObject = { ...sampleWithPartialData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.partialUpdate(patchObject).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PATCH' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should return a list of Funcionario', () => {
      const returnedFromService = { ...requireRestSample };

      const expected = { ...sampleWithRequiredData };

      service.query().subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush([returnedFromService]);
      httpMock.verify();
      expect(expectedResult).toMatchObject([expected]);
    });

    it('should delete a Funcionario', () => {
      const expected = true;

      service.delete(123).subscribe(resp => (expectedResult = resp.ok));

      const req = httpMock.expectOne({ method: 'DELETE' });
      req.flush({ status: 200 });
      expect(expectedResult).toBe(expected);
    });

    describe('addFuncionarioToCollectionIfMissing', () => {
      it('should add a Funcionario to an empty array', () => {
        const funcionario: IFuncionario = sampleWithRequiredData;
        expectedResult = service.addFuncionarioToCollectionIfMissing([], funcionario);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(funcionario);
      });

      it('should not add a Funcionario to an array that contains it', () => {
        const funcionario: IFuncionario = sampleWithRequiredData;
        const funcionarioCollection: IFuncionario[] = [
          {
            ...funcionario,
          },
          sampleWithPartialData,
        ];
        expectedResult = service.addFuncionarioToCollectionIfMissing(funcionarioCollection, funcionario);
        expect(expectedResult).toHaveLength(2);
      });

      it("should add a Funcionario to an array that doesn't contain it", () => {
        const funcionario: IFuncionario = sampleWithRequiredData;
        const funcionarioCollection: IFuncionario[] = [sampleWithPartialData];
        expectedResult = service.addFuncionarioToCollectionIfMissing(funcionarioCollection, funcionario);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(funcionario);
      });

      it('should add only unique Funcionario to an array', () => {
        const funcionarioArray: IFuncionario[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const funcionarioCollection: IFuncionario[] = [sampleWithRequiredData];
        expectedResult = service.addFuncionarioToCollectionIfMissing(funcionarioCollection, ...funcionarioArray);
        expect(expectedResult).toHaveLength(3);
      });

      it('should accept varargs', () => {
        const funcionario: IFuncionario = sampleWithRequiredData;
        const funcionario2: IFuncionario = sampleWithPartialData;
        expectedResult = service.addFuncionarioToCollectionIfMissing([], funcionario, funcionario2);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(funcionario);
        expect(expectedResult).toContain(funcionario2);
      });

      it('should accept null and undefined values', () => {
        const funcionario: IFuncionario = sampleWithRequiredData;
        expectedResult = service.addFuncionarioToCollectionIfMissing([], null, funcionario, undefined);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(funcionario);
      });

      it('should return initial array if no Funcionario is added', () => {
        const funcionarioCollection: IFuncionario[] = [sampleWithRequiredData];
        expectedResult = service.addFuncionarioToCollectionIfMissing(funcionarioCollection, undefined, null);
        expect(expectedResult).toEqual(funcionarioCollection);
      });
    });

    describe('compareFuncionario', () => {
      it('Should return true if both entities are null', () => {
        const entity1 = null;
        const entity2 = null;

        const compareResult = service.compareFuncionario(entity1, entity2);

        expect(compareResult).toEqual(true);
      });

      it('Should return false if one entity is null', () => {
        const entity1 = { id: 123 };
        const entity2 = null;

        const compareResult1 = service.compareFuncionario(entity1, entity2);
        const compareResult2 = service.compareFuncionario(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey differs', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 456 };

        const compareResult1 = service.compareFuncionario(entity1, entity2);
        const compareResult2 = service.compareFuncionario(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey matches', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 123 };

        const compareResult1 = service.compareFuncionario(entity1, entity2);
        const compareResult2 = service.compareFuncionario(entity2, entity1);

        expect(compareResult1).toEqual(true);
        expect(compareResult2).toEqual(true);
      });
    });
  });

  afterEach(() => {
    httpMock.verify();
  });
});
