...
import empresaResolve from './empresa-routing-resolve.service';

describe('Empresa routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: EmpresaService;
  let resultEmpresa: IEmpresa | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IEmpresa returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        empresaResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultEmpresa).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IEmpresa>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
