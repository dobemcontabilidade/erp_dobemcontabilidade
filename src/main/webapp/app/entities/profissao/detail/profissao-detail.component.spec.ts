...
describe('Profissao Management Detail Component', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [ProfissaoDetailComponent],
      providers: [
        provideRouter(
          [
            {
  ...
  describe('OnInit', () => {
    it('Should load profissao on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', ProfissaoDetailComponent);

      // THEN
      expect(instance.profissao()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
