...
describe('Escolaridade Management Detail Component', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [EscolaridadeDetailComponent],
      providers: [
        provideRouter(
          [
            {
              ...
              resolve: { escolaridade: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load escolaridade on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', EscolaridadeDetailComponent);

      // THEN
      expect(instance.escolaridade()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
