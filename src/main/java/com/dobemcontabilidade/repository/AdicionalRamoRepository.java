...
public interface AdicionalRamoRepository extends JpaRepository<AdicionalRamo, Long> {
    ...
    @Query(
        value = "select adicionalRamo from AdicionalRamo adicionalRamo left join fetch adicionalRamo.ramo left join fetch adicionalRamo.planoAssinaturaContabil",
    ...
    @Query(
        "select adicionalRamo from AdicionalRamo adicionalRamo left join fetch adicionalRamo.ramo left join fetch adicionalRamo.planoAssinaturaContabil"
    )
    List<AdicionalRamo> findAllWithToOneRelationships();

    @Query(
        "select adicionalRamo from AdicionalRamo adicionalRamo left join fetch adicionalRamo.ramo left join fetch adicionalRamo.planoAssinaturaContabil where adicionalRamo.id =:id"
    )
