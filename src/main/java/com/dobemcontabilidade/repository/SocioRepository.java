...
public interface SocioRepository extends JpaRepository<Socio, Long>, JpaSpecificationExecutor<Socio> {
    ...
    @Query(
        value = "select socio from Socio socio left join fetch socio.pessoa left join fetch socio.empresa",
        countQuery = "select count(socio) from Socio socio"
    )
    Page<Socio> findAllWithToOneRelationships(Pageable pageable);

    @Query("select socio from Socio socio left join fetch socio.pessoa left join fetch socio.empresa")
    List<Socio> findAllWithToOneRelationships();

    @Query("select socio from Socio socio left join fetch socio.pessoa left join fetch socio.empresa where socio.id =:id")
