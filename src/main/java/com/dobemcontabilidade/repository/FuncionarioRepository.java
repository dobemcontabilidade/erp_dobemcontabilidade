...
public interface FuncionarioRepository extends JpaRepository<Funcionario, Long>, JpaSpecificationExecutor<Funcionario> {
    ...
    @Query(
        value = "select funcionario from Funcionario funcionario left join fetch funcionario.pessoa left join fetch funcionario.empresa",
    ...
    @Query("select funcionario from Funcionario funcionario left join fetch funcionario.pessoa left join fetch funcionario.empresa")
    List<Funcionario> findAllWithToOneRelationships();

    @Query(
        "select funcionario from Funcionario funcionario left join fetch funcionario.pessoa left join fetch funcionario.empresa where funcionario.id =:id"
    )
