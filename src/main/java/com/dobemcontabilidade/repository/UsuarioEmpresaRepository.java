...
public interface UsuarioEmpresaRepository extends JpaRepository<UsuarioEmpresa, Long>, JpaSpecificationExecutor<UsuarioEmpresa> {
    ...
    @Query(
        value = "select usuarioEmpresa from UsuarioEmpresa usuarioEmpresa left join fetch usuarioEmpresa.pessoa left join fetch usuarioEmpresa.empresa",
    ...
    @Query(
        "select usuarioEmpresa from UsuarioEmpresa usuarioEmpresa left join fetch usuarioEmpresa.pessoa left join fetch usuarioEmpresa.empresa"
    )
    List<UsuarioEmpresa> findAllWithToOneRelationships();

    @Query(
        "select usuarioEmpresa from UsuarioEmpresa usuarioEmpresa left join fetch usuarioEmpresa.pessoa left join fetch usuarioEmpresa.empresa where usuarioEmpresa.id =:id"
    )
