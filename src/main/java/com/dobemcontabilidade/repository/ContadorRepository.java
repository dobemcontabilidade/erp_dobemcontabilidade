...
public interface ContadorRepository extends JpaRepository<Contador, Long>, JpaSpecificationExecutor<Contador> {
    ...
    @Query(
        value = "select contador from Contador contador left join fetch contador.pessoa left join fetch contador.perfilContador",
        countQuery = "select count(contador) from Contador contador"
    )
    Page<Contador> findAllWithToOneRelationships(Pageable pageable);

    @Query("select contador from Contador contador left join fetch contador.pessoa left join fetch contador.perfilContador")
    List<Contador> findAllWithToOneRelationships();

    @Query(
        "select contador from Contador contador left join fetch contador.pessoa left join fetch contador.perfilContador where contador.id =:id"
    )
