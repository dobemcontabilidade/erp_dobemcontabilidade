...
public interface EmpresaRepository
    ...
    @Query(
        value = "select empresa from Empresa empresa left join fetch empresa.empresaModelo",
        countQuery = "select count(empresa) from Empresa empresa"
    )
    Page<Empresa> findAllWithToOneRelationships(Pageable pageable);

    @Query("select empresa from Empresa empresa left join fetch empresa.empresaModelo")
    List<Empresa> findAllWithToOneRelationships();

    @Query("select empresa from Empresa empresa left join fetch empresa.empresaModelo where empresa.id =:id")
